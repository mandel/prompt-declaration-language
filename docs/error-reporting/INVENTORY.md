...
| E-SCHEMA-009 [obs] | list/object shape mismatch — and, on one shape of schema, **a crash inside the analyzer** | **Four behaviours, four corpus entries, three arms of `analyze_errors`.** (1) `prog.pdl:2 - result should be a list` + `  in contribute`, from the array arm's `not isinstance(data, list)` branch (`corpus/E-SCHEMA-009/`, `contribute: result`): the value is `str(data)`, undelimited, so the message reads as prose; nothing says a list of *what*, though `ContributeTarget`'s four values are in the same schema; no suggestion, though `contribute: [result]` is the whole fix. (2) `prog.pdl:2 - [{'greeting': 'hi'}] should be an object` + `  in defs`, from the object arm (`corpus/E-SCHEMA-009-object/`, `defs:` written as a list): same, plus a **Python `repr` of the parsed YAML** shown back to the user, and `object` is JSON-Schema's noun rather than PDL's. (3) `prog.pdl:2 - [{'text': 'recovering'}] should not be a list` + `  in fallback`, from the union arm's `found is None` branch (`corpus/E-SCHEMA-009-not-a-list/`, `fallback:` written as a sequence of blocks, which is how `text:` is written): reachable because `is_array` reads a literal `type: array` and so never sees an array behind a `$ref` — and this one states **no expectation at all**, only a prohibition. (4) **`KeyError: 'items'` — a hard crash out of the error reporter**, 76 lines of stderr, two chained tracebacks, 18 frames, the user's file named nowhere, and the raw pydantic `ValidationError` the analyzer was called to translate leaking out with it. The array arm subscripts `schema["items"]` after `is_array` has answered on `type` alone; `ExpressionFloatOrFloatFloat` (the type of `retry: {jitter:}`) renders one alternative as `type: array` + `prefixItems` and **no `items`**. `retry: {jitter: [1, 2, 3]}` is 3 lines and reproduces it (`corpus/E-SCHEMA-009-items-crash/`, `hygiene_traceback_expected`); `jitter: [1, 2]` is valid and does not. Present at `f0d91a1`, 30 hits in the 10 920-mutation sweep, see 7.11 | (1)–(3) file:line + block path, but the line is the **key's**, not the value's, so it is right only where the two share a line; **none at all** for the crash | **S0** (the crash; the three shape messages alone are S2) |
