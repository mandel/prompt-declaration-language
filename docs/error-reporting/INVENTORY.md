...
| E-RUNTIME-003 [obs] | included file has *any* static fault | **Open**, and now run rather than read. `process_include` (`pdl_interpreter.py:3834-3842`) wraps its whole body in one `try` and ends `except PDLParseError as exc: message = f"Attempting to include invalid yaml: {str(file)}\n{exc.text}"`. `PDLParseError` is the carrier for every static failure of an included file — YAML syntax error, schema error, duplicate key, and missing file (`PDLFileNotFoundError` subclasses it, `pdl_parser.py:97`) — so the prefix is **unconditional** and `yaml` is a guess that is right for one class in four. Two corrections to the `[src]` reading this row used to carry. The path is **relative**, not absolute: `str(state.cwd / block.include)` follows however `pdl` was invoked, so #410 applies to an absolute invocation and not to the ordinary one. And `import:` is no longer symmetric — `process_import` gained an `except OSError` ahead of the parse (`:3900`) that answers a missing file with no prefix, which is why E-RUNTIME-002 reads *cannot import `nosuch`: no such file* while E-RUNTIME-001, the same fault through `include:`, still reads `Attempting to include invalid yaml: does_not_exist.pdl`. The import wrapper itself survives at `:3944`. Sharpest case: a duplicate key, whose diagnostic was written *never* to say "not valid YAML" (5.5) because PyYAML accepts the document — the wrapper restores that claim one line above a paragraph conceding "Most YAML readers accept a key written more than once". Reproducers `tests/errors/corpus/E-RUNTIME-003/` (schema fault) and `E-RUNTIME-003-duplicate-key/` | outer file:line:col + block path, inner file:line:col + block path; the header's *claim* is what is wrong, not the coordinates | S2 |
